...
namespace BLINDED_AM_ME{
	...
	public class Path_Comp : MonoBehaviour {
		...
		public void Update_Path(bool forward)
        {
            ...
            {
                Vector3[] pointsR = new Vector3[points.Length];
                Vector3[] upsR = new Vector3[ups.Length];
                for (int i = 0; i < points.Length; i++)
                    pointsR[i] = points[ups.Length - 1 - i];
                for (int i = 0; i < ups.Length; i++)
                    upsR[i] = ups[ups.Length - 1 - i];
                if (transform.childCount > 1)
                {
                    _path.SetPoints(pointsR, upsR, isCircuit);
                }
            }
        }
