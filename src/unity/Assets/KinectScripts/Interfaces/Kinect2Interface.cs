...
//	private int faceDisplayHeight;
	...
	public KinectInterop.SensorData OpenDefaultSensor (KinectInterop.FrameSource dwFlags, float sensorAngle, bool bUseMultiSource)
	{
		...
		if((dwFlags & KinectInterop.FrameSource.TypeColor) != 0)
		{
            ...
            sensorData.colorImageBGRA = new byte[frameDesc.BytesPerPixel * frameDesc.LengthInPixels];
            sensorData.colorImagePostProc= new byte[frameDesc.BytesPerPixel * frameDesc.LengthInPixels];
        }
	...
	public bool PollColorFrame (KinectInterop.SensorData sensorData)
	{
		   ...
		   colorFrameReader != null) 
		{
			...
			if(colorFrame != null)
			{
                ...
                var pColorBGRAData = GCHandle.Alloc(sensorData.colorImageBGRA, GCHandleType.Pinned);
                var pColorPostData = GCHandle.Alloc(sensorData.colorImagePostProc, GCHandleType.Pinned);
                ...
                colorFrame.CopyConvertedFrameDataToIntPtr(pColorPostData.AddrOfPinnedObject(), (uint)sensorData.colorImagePostProc.Length, ColorImageFormat.Rgba);
                colorFrame.CopyConvertedFrameDataToIntPtr(pColorBGRAData.AddrOfPinnedObject(), (uint)sensorData.colorImageBGRA.Length, ColorImageFormat.Bgra);
                pColorPostData.Free();
                pColorData.Free();
                pColorBGRAData.Free();
